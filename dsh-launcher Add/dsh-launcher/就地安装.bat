...
if /i "%SKILL_DIR%"=="%AGENTS_SKILL%" (
    echo 检测到当前即技能目录，跳过复制与清理步骤，直接就地安装...
    goto :install
)
...
if errorlevel 1 (
    echo [错误] 复制技能目录失败（退出码 %ERRORLEVEL%），安装中止。
    pause
    exit /b 1
)
...
if /i not "%SKILL_DIR%"=="%AGENTS_SKILL%" if exist "%SKILL_DIR%\assets\install-dir.txt" copy /y "%SKILL_DIR%\assets\install-dir.txt" "%AGENTS_SKILL%\assets\install-dir.txt" >nul
echo.
